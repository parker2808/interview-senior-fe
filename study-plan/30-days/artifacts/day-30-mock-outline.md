# Day 30 — Mock interview

**Ngày:** 01/11/2026 · **Timebox:** 60–90 phút  
**Mục tiêu ngày:** Giải thích Capstone ~10–15′ không nhìn code; có weak-topic list.  
**Capstone link:** Rehearse từ [`capstone/15-delivery-notes.md`](./capstone/15-delivery-notes.md).

---

## Hướng dẫn (làm gì trong 60–90′)

1. **Đọc** full Self-check; skim 1 weak topic (`system-design` / `architecture` / EN `practical-questions`).
2. **Outline mock** 10–15′: product → state → API → a11y → test → AI → Vue vs React trade-off.
3. **Weak-topic list** + optional 5′ triage.
4. Không nhìn code khi rehearse.

---

## Đọc (đã verify trong repo)

| Nguồn | Path | Ghi chú |
|---|---|---|
| Self-check | [docs/self-check-questions.md](../self-check-questions.md) | Full |
| Weak topic (chọn 1) | `/workspace/src/vi/system-design.md` **hoặc** `/workspace/src/vi/architecture.md` **hoặc** `/workspace/src/en/practical-questions.md` | EN wording nếu chọn EN |
| Delivery notes | [capstone/15-delivery-notes.md](./capstone/15-delivery-notes.md) | Script gốc |

Plan Day 30: [30-day-study-plan.md — Day 30](../30-day-study-plan.md#day-30--01112026--mock-interview)

---

## 1. Mock outline (10–15′)

| Phút | Topic | Talking points (bullet) |
|---|---|---|
| 0–2 | Product / flow | |
| 2–4 | State ownership | |
| 4–6 | API + errors | |
| 6–8 | A11y | |
| 8–10 | Testing | |
| 10–12 | AI review | |
| 12–15 | Vue vs React spike trade-off | |

---

## 2. Anticipated questions

1.  
2.  
3.  
4.  
5.  

---

## 3. Weak-topic list

| Topic | Why weak | 1 hành động ôn |
|---|---|---|
| | | |
| | | |
| | | |

---

## 4. 5′ triage (optional)

Nếu còn thời gian trong tháng — làm gì?

> *(điền)*

---

## 5. Rehearsal log

| Lần | Thời lượng | Lúng túng chỗ nào |
|---|---|---|
| 1 | | |
| 2 | | |

---

## Checklist trước khi dừng

- [ ] Outline 10–15′
- [ ] ≥5 anticipated questions
- [ ] Weak-topic list


Quay lại: [Day 30 starter](../day-30-starter.md) · [Plan](../30-day-study-plan.md) · [Project context](../project-context.md)
